...
- `FUTURES_BALANCE_GUARD_BUFFER=0.95` and `FUTURES_BALANCE_GUARD_MAX_ATTEMPTS=2` retry a live MEXC entry once at a smaller contract size when the exchange reports order `cost` above available balance; if the reduced order would be below `minVol`, the trade is skipped instead of raising a runtime error.
...
Prediction-market overlays are opt-in with `FUTURES_PREDICTION_OVERLAY_ENABLED=true`. Live mode can read the current state from Redis key `FUTURES_PREDICTION_OVERLAY_REDIS_KEY` or from `FUTURES_PREDICTION_PRIMARY_URL` plus optional `FUTURES_PREDICTION_SECONDARY_URLS`. Backtests must use a point-in-time historical file via `FUTURES_BACKTEST_PREDICTION_STATE_FILE`; do not derive this file from future price moves or resolved outcomes.

Build the historical prediction file from timestamped Prophet/0G/Polymarket/Kalshi snapshot exports:

```powershell
python tools/build_prediction_state_file.py `
   --input data/prophet_snapshots.jsonl data/polymarket_rows.csv `
   --output data/prediction_overlay_20260430_20260530.json `
   --start 2026-04-30T07:00:00Z `
   --end 2026-05-30T07:00:00Z `
   --ttl-seconds 900 `
   --symbols BTC_USDT,ETH_USDT,SOL_USDT,BNB_USDT,SEI_USDT,ZEC_USDT
```

Accepted exports are CSV, JSON, or JSONL. Each row or snapshot needs a point-in-time timestamp such as `timestamp`, `generated_at`, `as_of`, `observed_at`, or `published_at`; an event id/title; a source/provider; symbol(s) or global scope; a direction such as `bullish`/`bearish`; and a probability field such as `probability`, `yes_price`, `prophet_probability`, `polymarket_probability`, or `kalshi_probability`. The builder rejects outcome/resolution columns by default to avoid look-ahead leakage.

Public Polymarket/Kalshi history can also be fetched directly when the markets are unambiguous directional crypto questions:

```powershell
python tools/export_public_prediction_history.py `
   --output data/prediction_overlay_public_20260430_20260530.json `
   --start 2026-04-30T07:00:00Z `
   --end 2026-05-30T07:00:00Z `
   --symbols BTC_USDT,ETH_USDT,SOL_USDT,BNB_USDT,SEI_USDT,ZEC_USDT `
...
This exporter intentionally skips ambiguous range, ETF, airdrop, launch, and market-cap questions because their `Yes` prices do not map cleanly to long/short futures bias.

ProphetMarket currently exposes public current crypto odds, so the bot can archive those snapshots going forward. This does not enable the trading overlay by itself; keep `FUTURES_PREDICTION_OVERLAY_ENABLED=false` until the archive has enough point-in-time data to beat the baseline backtest.

```powershell
python tools/archive_prophet_prediction_odds.py `
   --archive-output data/prophet_prediction_overlay.jsonl `
   --latest-output data/prophet_prediction_latest.json `
   --symbols BTC_USDT,ETH_USDT,SOL_USDT,BNB_USDT,SEI_USDT,ZEC_USDT
```

In production, set `FUTURES_PROPHET_ARCHIVE_ENABLED=true` to let the runtime append the same snapshots every `FUTURES_PROPHET_ARCHIVE_REFRESH_SECONDS` seconds. Use `FUTURES_PROPHET_ARCHIVE_FILE=/data/prophet_prediction_overlay.jsonl` on Railway so the archive survives restarts; `FUTURES_PROPHET_ARCHIVE_REDIS_KEY` can publish the latest state to Redis for later overlay testing.

Opportunity bucket sizing is available with `FUTURES_OPPORTUNITY_BUCKET_SIZING_ENABLED=true`. It maps the existing technical/event score to a 0-10 opportunity score, skips scores 0-5, uses 50% of available balance for scores 6-7, 75% for scores 8-9, and 100% for score 10. In that mode `FUTURES_OPPORTUNITY_MAX_LEVERAGE` defaults to `20`, `max_concurrent_positions` is forced to one, and `USE_NAV_RISK_SIZING=1` caps contracts by score-bucketed NAV risk before the bucket margin is spent: score 6 defaults to `1.5%` NAV, scores 7-9 to `4.0%`, and score 10 to `4.5%`. Set `FUTURES_SCORE_BUCKET_NAV_RISK_ENABLED=0` to restore the flat `FUTURES_OPPORTUNITY_NAV_RISK_PCT` behavior, or override each bucket with `FUTURES_OPPORTUNITY_NAV_RISK_PCT_SCORE6`, `..._SCORE7`, `..._SCORE8`, `..._SCORE9`, and `..._SCORE10`. Dynamic leverage is still risk-capped first: score-10 setups can only reach high leverage when the stop is tight enough, the symbol/signal caps allow it, and `stop_distance_pct × leverage` stays under `FUTURES_DYNAMIC_LEVERAGE_MAX_MARGIN_LOSS_PCT`. Sharp-event risk multipliers reduce this NAV risk budget as well as available margin.
